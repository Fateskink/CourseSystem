    ...
    public class SubjectController : Controller
    {
        ...
        private IPagedList<CommentDto> GetComment(int? page)
        {
            using (var unitOfWork = new UnitOfWork(new ELearningDbContext()))
            {
                ...
                {
                    var commentsChild = unitOfWork.Comments.Query(x => x.Status && x.ReplyId == par.Id).OrderBy(x => x.Id);

                    var comments = new List<CommentDto>();

                    foreach (var comment in commentsChild)
                    {
                        comments.Add(new CommentDto
                        {
                            ...
                            AvataUrl = comment.User.LinkAvata,
                ...
                var pageNumber = (page ?? 1);
                var pageSize = 4;
                return commentDtos.ToPagedList(pageNumber, pageSize) ?? new PagedList<CommentDto>(null, pageNumber, pageSize);
            }
        }
